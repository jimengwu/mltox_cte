                        ...
                        try:
                            ...
                            with ProcessPool(max_workers=4) as pool:
                                for num, fold in enumerate(folds):
                                    ...
                                    train_matrix = distance_matrix.iloc[
                                        fold[0], fold[0]
                                    ]
                            ...
                            if args.w_invitro == "own":
                                ...
                                max_euc = pd.DataFrame(
                                    euclidean_matrix(
                                        X_trainvalid, X_trainvalid, non_categorical
                                    )
                                ).values.max()

                                minmax = MinMaxScaler().fit(
                                    X_trainvalid[non_categorical]
                                )
                                X_trainvalid[non_categorical] = minmax.transform(
                                    X_trainvalid.loc[:, non_categorical]
                                )
                                X_valid[non_categorical] = minmax.transform(
                                    X_valid.loc[:, non_categorical]
                                )
                            ...
                            if args.w_invitro != "False":
                                if str(args.db_invitro) == "overlap":
                                    ...
                                    matrix_invitro_euc = pd.DataFrame(
                                        db_invitro_matrix[2]
                                    )
                            ...
                            #     names=["series_name"],
                            # )
                            # df_output["model"] = str(params_comb[j])
                            # # print(str(list(X_valid.index)))
                            # df_output["test_index"] = str(list(X_valid.index))
                            # df_output["test_cas"] = str(list(X_valid.test_cas.unique()))
                            try:
                                test_total = X_valid.shape[0]
                                test_correct = np.sum(
                                    X_valid.invitro_label.values == Y_valid
                                )
                                test_acc = test_correct / test_total
                                train_total = X_trainvalid.shape[0]
                                train_correct = np.sum(
                                    X_trainvalid.invitro_label.values == Y_trainvalid
                                )
                            ...
                            for k, v in params_comb[j].items():
                                temp_grid = pd.concat(
                                    [temp_grid, pd.DataFrame([v], columns=[k])], axis=1
                                )
                            temp_grid = pd.concat(
                                [temp_grid, pd.DataFrame([ah], columns=["ah"])], axis=1
                            )
                            temp_grid = pd.concat(
                                [temp_grid, pd.DataFrame([ap], columns=["ap"])], axis=1
                            )
                            temp_grid = pd.concat(
                                [
                                    temp_grid,
